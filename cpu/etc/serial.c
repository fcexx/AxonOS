...
uint32_t inportl(uint16_t _port) {
    uint32_t rv;
    /* Force 16-bit port (DX); plain "d" on x86-64 must pair with %w for inl/outl. */
    asm volatile ("inl %w1, %0" : "=a" (rv) : "d" (_port));
    return rv;
}
/*
 * Read 2 bytes
...
uint16_t inports(uint16_t _port) {
    uint16_t rv;
    asm volatile ("inw %w1, %0" : "=a" (rv) : "d" (_port));
    return rv;
}

void outports(uint16_t _port, uint16_t _data) {
    asm volatile ("outw %0, %w1" : : "a" (_data), "d" (_port));
}
...
void outportl(uint16_t _port, uint32_t _data) {
    asm volatile ("outl %0, %w1" : : "a" (_data), "d" (_port));
}